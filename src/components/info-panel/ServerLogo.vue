<template>
	<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48">
		<linearGradient id="5AaKlQpG8sfTCs926deHsa_c7wDY2TRDZHz_gr1" x1="24" x2="24" y1="39.744" y2="6.848"
			gradientTransform="matrix(1 0 0 -1 0 47.89)" gradientUnits="userSpaceOnUse">
			<stop offset="0" stop-color="#4c4c4c"></stop>
			<stop offset="1" stop-color="#343434"></stop>
		</linearGradient>
		<path fill="url(#5AaKlQpG8sfTCs926deHsa_c7wDY2TRDZHz_gr1)"
			d="M5,8h38c0.552,0,1,0.448,1,1v4c0,0.552-0.448,1-1,1l0,0c-1.222,0-2.337,0.701-2.866,1.803	L28.283,40.474C28.109,40.798,27.771,41,27.403,41h-4.785c-0.379,0-0.725-0.214-0.894-0.553l-10.935-22.87	C9.693,15.385,7.451,14,5,14l0,0c-0.552,0-1-0.448-1-1V9C4,8.448,4.448,8,5,8z">
		</path>
		<path fill="#fff" fill-rule="evenodd"
			d="M6,11.323V11c0-0.552,0.448-1,1-1h18c0.552,0,1,0.448,1,1v0.219	c0,0.459-0.312,0.859-0.757,0.97l-0.824,0.206c-1.241,0.31-1.876,1.69-1.304,2.835l4.345,8.69c0.199,0.398,0.781,0.357,0.922-0.065	l2.846-8.539c0.426-1.279-0.265-2.662-1.544-3.088l0,0C29.275,12.092,29,11.71,29,11.279V11c0-0.552,0.448-1,1-1h11.061	C41.579,10,42,10.421,42,10.939v0.384c0,0.404-0.259,0.763-0.642,0.891l0,0c-1.51,0.503-2.758,1.586-3.469,3.009L27.267,38.416	C27.104,38.772,26.749,39,26.358,39h-2.729c-0.385,0-0.735-0.221-0.902-0.567l-9.874-20.571c-1.192-2.484-3.273-4.43-5.831-5.453	l-0.394-0.158C6.249,12.1,6,11.732,6,11.323z"
			clip-rule="evenodd"></path>
		<linearGradient id="5AaKlQpG8sfTCs926deHsb_c7wDY2TRDZHz_gr2" x1="14.755" x2="36.92" y1="40.623" y2="18.457"
			gradientTransform="matrix(1 0 0 -1 0 47.89)" gradientUnits="userSpaceOnUse">
			<stop offset="0" stop-color="#4caf50"></stop>
			<stop offset=".133" stop-color="#4aae50"></stop>
			<stop offset=".239" stop-color="#43a94e"></stop>
			<stop offset=".373" stop-color="#38a14c"></stop>
			<stop offset=".482" stop-color="#279648"></stop>
			<stop offset=".621" stop-color="#184"></stop>
			<stop offset="1" stop-color="#088242"></stop>
		</linearGradient>
		<path fill="url(#5AaKlQpG8sfTCs926deHsb_c7wDY2TRDZHz_gr2)"
			d="M39.609,11h-8.338c-0.317,0-0.417,0.427-0.134,0.568l0.394,0.197	c1.396,0.698,2.023,2.349,1.444,3.797l0,0l0,0L31.6,19l-3.201,8.002c-0.158,0.395-0.705,0.425-0.906,0.05l-6.044-11.333	c-0.794-1.489-0.204-3.34,1.305-4.095c0.31-0.155,0.2-0.623-0.147-0.623H9.496c-0.42,0-0.573,0.553-0.213,0.769	c2.422,1.453,4.347,3.604,5.524,6.172l8.926,19.475C23.896,37.772,24.251,38,24.642,38h0.703c0.398,0,0.758-0.236,0.917-0.601	l9.405-21.621l0,0c0.865-1.802,2.305-3.265,4.094-4.158C40.07,11.465,39.954,11,39.609,11z">
		</path>
	</svg>
</template>